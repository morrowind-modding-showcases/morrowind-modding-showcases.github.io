...
2. The browser builds a structured version-4 payload with a required public contributor name, shows a sanitized Markdown review, obtains a single-use Turnstile token, and posts to the Worker. The validator still normalizes queued version-1 through version-3 payloads.
...
4. The Action verifies and decodes the payload, revalidates it against the checked-out repository, reconstructs the primary wiki page, any doormarker-derived location pages, and one contribution audit record, validates the entire site, and opens a public pull request.
...
The public browser never receives a GitHub token and never writes to GitHub directly. The Worker's fine-grained GitHub token can dispatch only the trusted workflow; repository write credentials remain in GitHub Actions. A submission has exactly one primary wiki page and may include new location pages derived from that mod's plugin. Contributor identity is public and survives review in `content/wiki-contributions/<submission UUID>.json`. New records use schema version 2 and explicitly distinguish a free-form external contributor from a selected stable modder profile ID. The optional **Remember user name** preference stores only that name in a secure, site-scoped cookie for one year; no article form state is stored.

## Public flow

The navigation exposes **Mods | Locations | Resources | Leaderboard | Recent changes | Contribute**. The Contribute page offers **Add a new mod page**. Individual mod and mapped-location articles expose **Suggest an edit**; indexes, folder pages, tag pages, the wiki home, and the Contribute page do not. There is no standalone public new-location submission kind: new locations are proposed from a main or component plugin while submitting a mod.
...
placement choice. The contributor can keep the current main geometry and add a
variant for that main or component plugin, or promote the plugin geometry to
the one main location. A promotion preserves the former main geometry as a
variant for its recorded source. The importer independently verifies the
threshold, mod-added status, coverage, component, and plugin before updating
the location article.

A complete HTTP(S) download URL is required for every mod contribution. Uploading a plugin only helps populate its map locations and exterior edits; contributors still provide the mod metadata in the same form. The article field opens in an editable formatted view, offers icon-only controls for common Markdown formatting, and includes a Markdown source mode plus a link to Obsidian's syntax reference. Typing `[[` or using the internal-link button searches published mod and location titles and inserts a canonical path such as `[[locations/balmora|Balmora]]`. Preview and formatted-editor rendering build sanitized DOM nodes and treat submitted HTML as text. Description frontmatter is not submitted; Quartz derives page and SEO descriptions from article text. The review screen can download the complete generated Markdown using the generated filename for new pages or the repository filename for edits. The contributor field defaults to external-contributor mode, where it accepts a history-generated suggestion or any new public display name. Enabling **Modder profile** switches it to a strict profile combobox: a real result must be selected and the submission stores that profile's canonical name and stable ID. Typed-but-unselected profile text is invalid, and switching modes clears incompatible identity state. There are no private-note, contact, tag, GitHub-login, or direct-edit fields. The form clearly states that the resulting pull request is public. A failed API request keeps the in-memory form state and resets Turnstile for a new token. A successful request saves or clears the optional name cookie, clears the remaining state, and reports that PR creation is pending validation.
...
New mods are written only when the validated slug does not exist. Their reconstructed frontmatter has one category, `draft: false`, no tags, and no automatic article heading. Public standalone `new-location` payloads remain rejected. Validated mod submissions may carry bounded `new_locations` entries; the importer creates those location Markdown files with deterministic collision-checked map IDs, generic marker metadata, `mod_added: true`, and the mod slug in `mod_added_by`. Version-4 submissions may also carry bounded `location_variants` choices; these update only controlled, existing mod-added location articles and retain one top-level main geometry.

After all validation passes, the Action commits only the intended primary page, validated new location pages, and matching contribution record, pushes a unique `wiki-submission-<uuid>-<run>-<attempt>` branch, and opens a normal public PR. Its title and body contain no encoded payload or untrusted contributor text; the public contributor name appears in the audit-record diff. The PR's Git diff is the single canonical human review surface. Closing a rejected PR keeps the record out of `main`; merging an accepted PR makes it part of the generated contributor list, leaderboard, and recent-changes feed on the normal deployment.

## Order identity and scores

Modder records may declare explicit legacy wiki identities under `wiki.contributorNames`. Matching trims and collapses whitespace, applies Unicode NFKC normalization, and compares case-insensitively. Generic modder aliases remain useful for profile search but do not link wiki history. A normalized wiki identity claimed by two profiles is a validation error naming both profiles.

`assets/data/order-scores.json` is generated from the central modder registry and accepted contribution history. Direct version-2 `modderId` credits are authoritative; legacy and external-name records resolve retroactively through `wiki.contributorNames`. Every resolved record adds one permanent Order Score point and grants the permanent Mark of Order. One contribution activity event per UTC day adds 20 Orderliness, capped at 100; between events it decays exponentially toward a 1% floor using the rules in `content/order-rules.json`. Current display values are calculated client-side from the stored last-activity state and timestamp, so daily generated-file commits are unnecessary.

The same generated document includes an informational `unlinkedContributors` summary. Unlinked names remain external autocomplete suggestions and disappear automatically once an explicit wiki identity mapping resolves them.
...
- Update location articles and their controlled metadata through the existing wiki location workflow. The contribution asset uses `loadControlledVocabularies` and `canonicalMapLocations`; do not copy the vocabulary into client code. When a plugin adds several cells inside one unpublished settlement (for example `Bo-muul, …`), the map already clusters them under a synthesized container; publishing an optional parent location article titled with the shared prefix (for example `Bo-muul`) upgrades that cluster to a real marker automatically.
