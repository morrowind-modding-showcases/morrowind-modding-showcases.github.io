...
2. The browser builds a structured version-3 payload with a required public contributor name, shows a sanitized Markdown review, obtains a single-use Turnstile token, and posts to the Worker. The validator still normalizes queued version-1 and version-2 payloads.
