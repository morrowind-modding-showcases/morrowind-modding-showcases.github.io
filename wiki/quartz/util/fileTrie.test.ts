...
describe("FileTrie", () => {
  ...
  describe("add", () => {
    ...
    test("should merge a landing page with its folder regardless of insertion order", () => {
      const landing = {
        title: "Vivec",
        slug: "locations/vivec",
        filePath: "locations/vivec.md",
      };
      const child = {
        title: "Arena Underworks",
        slug: "locations/vivec/arena-underworks",
        filePath: "locations/vivec/arena-underworks.md",
      };
      const folderIndex = {
        title: "Vivec locations",
        slug: "locations/vivec/index",
        filePath: "locations/vivec/index.md",
      };

      for (const files of [
        [landing, child],
        [child, landing],
        [folderIndex, child, landing],
      ]) {
        const trie = new FileTrieNode<TestData>([]);
        files.forEach((file) => trie.add(file));
        const locations = trie.children[0];
        const vivecNodes = locations.children.filter(
          (node) => node.slugSegment === "vivec",
        );

        assert.strictEqual(vivecNodes.length, 1);
        assert.strictEqual(vivecNodes[0].isFolder, true);
        assert.strictEqual(vivecNodes[0].data, landing);
        assert.strictEqual(vivecNodes[0].displayName, "Vivec");
        assert.strictEqual(vivecNodes[0].children[0].data, child);
      }
    });
  });
