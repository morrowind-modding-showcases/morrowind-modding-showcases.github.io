---
title: "Rise of House Telvanni"
authors:
  - "Pozzo"
  - "Karpik777"
  - "Bhl"
url: "https://www.nexusmods.com/morrowind/mods/27545"
categories:
  - "Quests"
map_enabled: true
map_locations:
  - "Abandoned Housepod"
  - "Ald-ruhn, Codus Callonus: Bookseller"
  - "Ald-ruhn, Council Club"
  - "Ald-ruhn, Guild of Mages"
  - "Ald-ruhn, Telvanni Consulate"
  - "Aleft"
  - "Ammardunibi Camp, Bodrusa's Yurt"
  - "Ashmelech"
  - "Balmora, Guild of Mages"
  - "Balmora, Telvanni Consulate"
  - "Caldera, Guild of Mages"
  - "Caldera, Shenk's Shovel"
  - "Dagon Fel, Watch Tower"
  - "Dren Plantation, Dren's Villa"
  - "Drinith Ancestral Tomb"
  - "Ebernanit, Shrine"
  - "Ebonheart, Argonian Mission"
  - "Ebonheart, East Empire Company Hall"
  - "Ebonheart, Garos Relvani's Abode"
  - "Ebonheart, Grand Council Chambers"
  - "Ebonheart, Telvanni Consulate"
  - "Ebonheart, Underground Caves"
  - "Eluba-Addon Egg Mine"
  - "Endalos Ancestral Tomb"
  - "Gnisis, Madach Tradehouse"
  - "Nchuleft"
  - "Outer Realms, Nueranar"
  - "Sadrith Mora, Fara's Hole in the Wall"
  - "Sadrith Mora, Tel Naga Great Hall"
  - "Sadrith Mora, Telvanni Council House, Entry"
  - "Sadrith Mora, Wolverine Hall"
  - "Solstheim, Frossel"
  - "Solstheim, Himmelhost Barrow"
  - "Sud"
  - "Tel Aruhn, Upper Tower"
  - "Tel Azura, Greenhouse"
  - "Tel Dwemeris, Corridor"
  - "Tel Dwemeris, Globe of Construction"
  - "Tel Dwemeris, Manor"
  - "Tel Koj-Ruskthss, Lower Tower"
  - "Tel Koj-Ruskthss, Upper Tower"
  - "Tel Llarelah, Main Tower"
  - "Tel Mora, Upper Tower"
  - "Telasero, Dome"
  - "Telasero, Propylon Chamber"
  - "Telasero, Upper Level"
  - "Tureynulal, Kagrenac's Library"
  - "Vivec, Foreign Quarter Plaza"
  - "Vivec, Guild of Mages"
  - "Vivec, St. Delyn Canal South-One"
  - "Vivec, The Lizard's Head"
  - "Zergonipal, Shrine"
map_exterior_edits:
  - cell: "-2, 6"
    landscape: false
    references: 11
  - cell: "1, -12"
    landscape: false
    references: 1
  - cell: "0, 13"
    landscape: false
    references: 83
  - cell: "5, 3"
    landscape: false
    references: 2
  - cell: "15, 2"
    landscape: false
    references: 2
  - cell: "18, -8"
    landscape: false
    references: 4
  - cell: "19, -8"
    landscape: false
    references: 41
  - cell: "-3, -2"
    landscape: false
    references: 9
  - cell: "-2, 2"
    landscape: false
    references: 2
  - cell: "7, 22"
    landscape: false
    references: 17
  - cell: "1, -13"
    landscape: false
    references: 67
  - cell: "2, 4"
    landscape: false
    references: 26
  - cell: "-11, 11"
    landscape: false
    references: 1
  - cell: "8, 13"
    landscape: false
    references: 2
  - cell: "0, 14"
    landscape: false
    references: 17
  - cell: "6, 21"
    landscape: false
    references: 2
  - cell: "8, 12"
    landscape: false
    references: 89
  - cell: "18, 4"
    landscape: false
    references: 35
  - cell: "17, 4"
    landscape: false
    references: 8
  - cell: "2, 19"
    landscape: false
    references: 1
  - cell: "5, 22"
    landscape: false
    references: 2
  - cell: "6, 22"
    landscape: false
    references: 36
  - cell: "7, 21"
    landscape: false
    references: 2
  - cell: "15, 5"
    landscape: false
    references: 10
  - cell: "15, -13"
    landscape: false
    references: 1
  - cell: "15, 1"
    landscape: false
    references: 1
  - cell: "13, 14"
    landscape: false
    references: 3
  - cell: "10, 14"
    landscape: false
    references: 2
  - cell: "9, -7"
    landscape: false
    references: 100
  - cell: "10, 1"
    landscape: false
    references: 1
  - cell: "4, -11"
    landscape: false
    references: 5
  - cell: "11, 14"
    landscape: false
    references: 1
  - cell: "-14, 13"
    landscape: false
    references: 8
  - cell: "18, 3"
    landscape: false
    references: 1
draft: false
events: []
picture_url: "https://staticdelivery.nexusmods.com/mods/100/images/27545-2-1254943747.jpg"
---
> This is a heavily expanded version of Pozzo's Rise of House Telvanni (RoHT). This mod adds quests for the Telvanni Archmagister, allowing them to gain more political power and strengthen House Telvanni. During RoHT's main quest, the player will decide on House Telvanni's stance towards the Mages Guild and the Twin Lamps, infiltrate the East Empire Company, and shake Vvardenfell's politics to the core.
>
> Most quests can be played in different styles. Depending on the player's decisions, House Telvanni will gain anything between one and four new strongholds. It is up to the player if House Telvanni rises to political predominance or passes up its opportunities.
>
> RoHT also introduces some lore about the Telvanni and a chance to reclaim some ancient Telvanni artifacts.

## World Edits

Adds a number of new locations, including to the outskirts of major cities, with new structures and associated clutter and detailing. Also modifies a number of vanilla interiors, largely with quest objects and NPCs, but with occasional more extensive edits. Makes NO landscape edits.

## Other Notes

Variable compatibility with other mods but a number of compatibility patches exist, covering most popular mods. It's recommended that you use [Rise of House Telvanni 2.0](https://www.nexusmods.com/morrowind/mods/48225) for modern installs.