---
title: "Ald'ruhn - City of Ash"
authors:
  - "SuffersParadise"
url: "https://www.nexusmods.com/morrowind/mods/57520"
categories:
  - "Towns and Cities"
map_enabled: true
map_locations:
  - "Ald-ruhn, Ald Skar Inn"
  - "Ald-ruhn, Aryni Orethi's House"
  - "Ald-ruhn, Bevene Releth: Clothier"
  - "Ald-ruhn, Braynas Hlervu's House"
  - "Ald-ruhn, Codus Callonus: Bookseller"
  - "Ald-ruhn, Council Club"
  - "Ald-ruhn, Dandera Selaro: Smith"
  - "Ald-ruhn, Daynes Redothril: Pawnbroker"
  - "Ald-ruhn, Dravasa Andrethi's House"
  - "Ald-ruhn, Drinar Varyon's House"
  - "Ald-ruhn, Galthragoth's House"
  - "Ald-ruhn, Gildan's House"
  - "Ald-ruhn, Gindrala Hleran's House"
  - "Ald-ruhn, Guard Tower 1"
  - "Ald-ruhn, Guard Tower 2"
  - "Ald-ruhn, Guild of Fighters"
  - "Ald-ruhn, Guild of Mages"
  - "Ald-ruhn, Hanarai Assutlanipal's House"
  - "Ald-ruhn, Ienas Sarandas's House"
  - "Ald-ruhn, Ilmiril's House"
  - "Ald-ruhn, Manor District"
  - "Ald-ruhn, Pellecia Aurrus's House"
  - "Ald-ruhn, Tauryon's House"
  - "Ald-ruhn, Temple"
  - "Ald-ruhn, The Rat In The Pot"
  - "Ald-ruhn, Tiras Sadus: General Merchandise"
  - "Buckmoth Legion Fort, Interior"
  - "Buckmoth Legion Fort, Prison"
  - "Buckmoth Legion Fort, Towers"
  - "Morvayn Manor"
map_exterior_cells:
  - "-1, 6"
  - "-2, 7"
  - "-2, 6"
  - "-3, 6"
  - "-1, 7"
  - "-2, 8"
  - "-3, 5"
  - "-3, 7"
  - "-3, 8"
  - "-2, 5"
  - "-2, 4"
draft: false
events: []
picture_url: "https://staticdelivery.nexusmods.com/mods/100/images/57520/57520-1759006378-943566378.png"
showcase_url: "https://youtu.be/U3OmUm65iUs"
---
Overhaul of Ald'ruhn and all its interiors.

## Features:

- All interiors updated with more clutter and Tamriel_Data objects
- Many interiors changed
- New interiors
- New NPC's for new areas and for expanded vanilla ones
- Merchant inventories updated with TD items and lists
- Some vanilla NPC's have been given new outfits, and a small handful have new faces/hair from TD
- New Redoran city walls from TD
- A natural rock wall now surrounds much of the city

## World Edits:

Overhauls all exterior cells and almost all interior cells of Ald'ruhn and its surrounding area.

Makes landscape edits to the following cells:
- -1, 5
- -1, 6
- -1, 7
- -1, 8
- -2, 4
- -2, 5
- -2, 6
- -2, 7
- -2, 8
- -3, 4
- -3, 5
- -3, 6
- -3, 7
- -3, 8

## Other Notes:

Merges the following mods:

- Ald-ruhn Temple Expansion
- Ald-ruhn Mages Guild Expansion
- Edwinna's Study
- Ald Skar Inn Overhaul
- The OTHER Council Club
- Redoran Council Overhall

Will conflict with any other Ald'ruhn city overhaul mods.